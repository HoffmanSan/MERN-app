...
export default function ProductCard({product}: CardProps) {
  return (
    <Link to={`/products/${product._id}`}>
      <div className="flex flex-col items-center m-2 transition duration-200 ease-out rounded-md shadow-md min-h-96 group">
        <div className="p-2 bg-gray-100 rounded-md">
          <img
            src={product.photoURLs[0]}
            alt={product.name}
            loading="lazy"
            className="object-scale-down w-64 h-64 transition duration-200 ease-out group-hover:scale-105"
          />
        </div>
